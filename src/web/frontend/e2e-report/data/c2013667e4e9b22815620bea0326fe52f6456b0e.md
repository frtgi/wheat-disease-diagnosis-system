...
- Name: diagnosis.spec.ts >> 诊断流程 E2E 测试 >> 图像上传区域可见
- Location: e2e\diagnosis.spec.ts:18:3
...
Locator: locator('[class*="upload"], [class*="drop-zone"], [class*="image-upload"], .el-upload').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('[class*="upload"], [class*="drop-zone"], [class*="image-upload"], .el-upload').first()
