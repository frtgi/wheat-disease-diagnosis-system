...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    try:
        ...
        logger.error(f"密码验证失败 - 无效的密码格式: {e}")
        return False
...
def create_refresh_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    ...
    生成用于刷新访问令牌的 JWT，有效期比访问令牌更长（默认7天）。
    刷新令牌通常存储在更安全的位置（如 HttpOnly Cookie），
    用于在访问令牌过期后获取新的访问令牌，无需用户重新登录。

    参数:
        data: 令牌载荷数据，通常包含用户标识（如 sub 字段）
        expires_delta: 自定义过期时间增量，默认为 REFRESH_TOKEN_EXPIRE_DAYS 天
...
def verify_token(token: str) -> Optional[dict]:
    ...
    返回:
        Optional[dict]: 验证成功返回令牌载荷字典，失败返回 None
    """
    try:
        payload = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM])
        return payload
...
async def get_current_user(
    ...
    支持从 Authorization header 和 httpOnly cookie 两种方式获取 Token，
    确保向后兼容的同时提升安全性。

    参数:
        token: JWT 令牌（从 header 或 cookie 获取）
        db: 数据库会话
    ...
    if await is_token_blacklisted(token):
        logger.warning(f"检测到黑名单 Token，拒绝访问: username={username}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="令牌已失效，请重新登录",
            headers={"WWW-Authenticate": "Bearer"},
...
async def require_admin(current_user: User = Depends(get_current_user)) -> User:
    ...
    异常:
        HTTPException: 用户非管理员时返回 403 权限不足错误
    """
...
async def add_token_to_blacklist(token: str, ttl: int = None) -> bool:
    ...
    将 Token 添加到 Redis 黑名单中，防止被继续使用。
    ...
    返回:
        bool: 成功加入黑名单返回 True，失败返回 False
    ...
    try:
        ...
        logger.error(f"将 Token 加入黑名单失败: {e}", exc_info=True)
        return False
