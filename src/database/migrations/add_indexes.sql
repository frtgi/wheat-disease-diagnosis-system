...
-- ====================================================
-- 添加病害名称索引（用于统计和筛选）
ALTER TABLE `diagnosis_records` ADD INDEX `idx_disease_name` (`disease_name`);

-- 添加状态索引（用于筛选未完成的诊断）
ALTER TABLE `diagnosis_records` ADD INDEX `idx_status` (`status`);

-- 添加生长阶段索引（用于生长阶段分析）
ALTER TABLE `diagnosis_records` ADD INDEX `idx_growth_stage` (`growth_stage`);
...
ALTER TABLE `diagnosis_records` ADD INDEX `idx_user_created` (`user_id`, `created_at`);

-- 添加复合索引：病害名称 + 创建时间（用于病害趋势分析）
ALTER TABLE `diagnosis_records` ADD INDEX `idx_disease_created` (`disease_name`, `created_at`);
...
SHOW INDEX FROM `diagnosis_records`;
...
-- EXPLAIN SELECT * FROM diagnosis_records WHERE user_id = 1 ORDER BY created_at DESC LIMIT 10;

-- 查询 2: 按病害名称统计（使用 idx_disease_name）
-- EXPLAIN SELECT disease_name, COUNT(*) FROM diagnosis_records GROUP BY disease_name;
